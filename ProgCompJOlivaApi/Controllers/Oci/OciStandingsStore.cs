...
/// <summary>
/// Persists OCI standings as JSON files under <c>wwwroot/oci-standings/{key}.json</c>.
/// </summary>
public static class OciStandingsStore
{
    ...
    private static string PathFor(IWebHostEnvironment env, string key)
        => Path.Combine(env.WebRootPath, "oci-standings", $"{SanitizeKey(key)}.json");

    public static bool Exists(IWebHostEnvironment env, string key)
        => File.Exists(PathFor(env, key));

    public static async Task SaveAsync(IWebHostEnvironment env, string key, OciStandings standings, CancellationToken ct = default)
    {
        var dir = Path.Combine(env.WebRootPath, "oci-standings");
    ...
    public static async Task<OciStandings?> ReadAsync(IWebHostEnvironment env, string key, CancellationToken ct = default)
    {
        ...
        return JsonSerializer.Deserialize<OciStandings>(json, JsonOptions);
    }
